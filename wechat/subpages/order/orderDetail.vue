	...
	<view>
		...
		<view class="address-section">
			<view class="order-content">
				...
				<view class="cen">
					<view class="top">
						<text class="name">{{order.receiveAddress.name}}</text>
						<text class="mobile">{{order.receiveAddress.telephone}}</text>
					</view>
					<text class="address">{{order.receiveAddress.province}} {{order.receiveAddress.city}}
						{{order.receiveAddress.region}}
						{{order.receiveAddress.detailAddress}}</text>
				</view>
			</view>
		...
		methods: {
			...
			async loadData() {
				...
				fetchOrderDetail({
					...
					if (response.code == 0) {
						...
						_this.orderPayment = response.data.payment
						_this.order.orderItemList.forEach(item => {
			...
			payOrder(orderId) {
				...
				if (this.order.status == 0) {
					console.log("----orderPayment-----", _this.orderPayment)
					wx.requestPayment({
						"provider": 'wxpay',
