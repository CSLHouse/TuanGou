	...
	<view class="container">
		...
		<view class="share-section">
			...
			<button class="share-btn" open-type="share">
				<image src="/static/temp/share_wechat.png" mode="aspectFit"
		...
		<view class="detail-desc">
			...
			<!-- <div style="width:100%">
				<video style="width:100%;display:block;" v-for="(item, index) in info.vdDetail" :key="index" :src="item"
					controls="controls" mode="widthFix"></video>
				<img style="width: 100%;display:block;" v-for="(item, index) in info.imgDetail" :key="index" :src="item"
					mode="widthFix" @tap="_previewImage(item)" />
			</div>
		...
		<view class="page-bottom">
			...
			<view @click="handleClickBuy">
				...
				<view v-if="product.sale <= 0">
					<view class="action-btn-group">
						<view class="btn-text">跟团购买</view>
					</view>
				</view>
				<view class="action-btn-group" v-else>
					<view class="action-btn-left">
						<folding-avatar :avatars="avatarsList" :maxAvatars="product.sale"></folding-avatar>
					</view>
					<view class="action-btn-right">
						<view class="btn-text-top">跟团购买</view>
						<view class="btn-text-bottom">{{ product.sale }}人已跟团</view>
					</view>
				</view>
			</view>
		</view>
		...
		<view class=" popup spec" :class="specClass" @touchmove.stop.prevent="stopPrevent" @click="buy_before(0)">
			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				...
				<view class="item-list-number">
					<text>购买数量:</text>
					<uni-number-box :min="1" :max="100" :value="number" :isMax="false" :isMin="number===1"
						@eventChange="numberChange"></uni-number-box>
				</view>

				<view class="btn-group">
					<button class="btn-group-buy" @click="buy_before(1)">立即购买</button>
					<button class="btn-group-cart" @click="buy_before(2)">加入购物车</button>
				</view>
...
<script>
	...
	import foldingAvatar from '@/subpages/components/folding-avatar.vue';
		...
		methods: {
			async loadData(id) {
				fetchProductDetail(id).then(response => {
					...
					this.avatarsList = response.data.buyersList.avatars
					this.initImgList();
					this.initServiceList();
					this.initSpecList(this.product);
					this.initAttrList(this.product);
					this.initPromotionTipList(this.product);
			...
			initSpecList(data) {
				...
				for (let i = 0; i < this.skuStockList.length; i++) {
					if (this.skuStockList[i].spData.length > 0) {
			...
			initPromotionTipList(data) {
				...
				if (promotionType == 0) {
					...
					for (let item of data.productFullReductionList) {
						this.promotionTipList.push("满" + numFilter(item.fullPrice) + "元减" + numFilter(item.reducePrice) +
							"元");
					}
			...
			initProductDesc() {
				...
				// 使用正则表达式给图片添加样式
				rawhtml = rawhtml.replace(/<img/g, '<img style="width:100%;height:auto;display:block;"');
				this.desc = rawhtml
