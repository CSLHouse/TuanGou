  ...
  const init = reactive({
    ...
    images_upload_handler: (blobInfo: any, progress: any) => new Promise((resolve, reject) => {
      ...
      if (blobInfo.blob().size / 1024 / 1024 > 2) {
        ...
        let url = props.baseUrl ? props.baseUrl : import.meta.env.VITE_BASE_PATH + ":" + import.meta.env.VITE_CLI_PORT + import.meta.env.VITE_BASE_API + "/fileUploadAndDownload/upload"
        axios.defaults.timeout = 99999 //设置超时时间为10秒
        axios.post(url, params, config).then(res => {
          if (res.data.code == 0) {
            if (!res.data.data.file || !res.data.data.file.url) {
              reject('上传失败，图片路径不存在')
              return
            }
            resolve(res.data.data.file.url)  //上传成功，在成功函数里填入图片路径
